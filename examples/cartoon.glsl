...
{
    highp float luminance = dot(color.rgb, LUM_VECTOR);
...
mediump vec4 curves(mediump vec4 base)
{
    return vec4(cartoon_rgb(base.rgb), base.a);
}

void main(void) {
    gl_FragColor = texture2D(texture, vUv);
    highp vec4 color = curves(gl_FragColor);
    highp vec4 edge = vec4(vec3(1) - edge(), 1.0);
    color *= edge;
